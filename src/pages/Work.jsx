...
import { Sparkles, Calendar, Layers, ShieldCheck, Award, Home, Compass } from 'lucide-react';
...
const Work = () => {
  ...
  const fetchWorkProjects = async () => {
    try {
      ...
      console.warn('Backend work fetch warning using rich dummy projects fallback:', err);
      let filtered = DUMMY_WORK_PROJECTS;
      if (selectedRoom !== 'All') {
        filtered = DUMMY_WORK_PROJECTS.filter(p => p.roomType === selectedRoom);
      }
  ...
  return (
    <div className="pt-28 pb-20 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative overflow-hidden">
      ...
      {/* Room Category Tabs */}
      <div className="flex flex-wrap items-center gap-2 mb-10 pb-4 border-b border-[#E3DDCE] relative z-10">
